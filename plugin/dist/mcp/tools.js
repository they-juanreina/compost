...
export const TOOLS = [
    ...
    },
    {
        ...
        description: "Write an analytic memo — your dated interpretive note about the corpus, a code, or a theme (the reasoning behind a move, a reflexive observation, a pattern hunch). Anchor it to what it's about. Always provide a concise, evocative, retrieval-friendly `title` you generate from the content (the human may have only given you a raw thought). Lands as an AI [draft] until a researcher endorses (compost endorse) — propose, never self-approve.",
        readOnly: false,
        aiAuthored: true,
        inputSchema: {
            type: 'object',
            ...
            properties: {
                ...
                title: str('A short retrieval-friendly title. Optional in the CLI, but you should generate one from the content for scannability (the human edits it on endorse if needed).'),
                type: {
                    type: 'string',
    ...
    },
    {
        ...
        description: "List analytic memos (newest first). Filter by `about` (memos anchored to an artifact — read the analyst's notes on a code/theme before acting), `type`, or `codebook`.",
        readOnly: true,
        inputSchema: {
            type: 'object',
            properties: {
    ...
    },
    {
        ...
        description: 'Create a codebook — an interpretive lens codes belong to (ADR 0001) — with a declared stance. A seed can hold several coexisting lenses over one corpus; codes are scoped to one. Researcher-authored (structural setup), not an AI [draft].',
        readOnly: false,
    ...
    },
    {
        ...
        description: "List the seed's codebooks (lenses) with their stance — the implicit `primary` plus any created lenses.",
        readOnly: true,
        inputSchema: { type: 'object', properties: { seed: str('Seed') } },
    ...
    },
    {
        ...
        description: 'Assign codes that predate codebooks to the primary codebook. Dry-run by default (previews the affected codes); set apply=true to emit the update events and stamp frontmatter.',
        readOnly: false,
    ...
    },
    {
        ...
        description: 'Merge one codebook into another (ADR 0001, #269). Re-homes the source frame’s codes into the target (an update, not a copy — identity + evidence + history preserved), then reject-archives the source (never deletes). Colliding code names are kept distinct (`distrust` → `distrust-from-<frame>`), never silently fused — de-dup is a separate explicit step. Dry-run by default (previews the re-home + any blocking refs); set apply=true to write. Refuses when a re-homing code is cited by a theme or category link — resolve those first. Researcher-authored, not an AI [draft].',
        readOnly: false,
